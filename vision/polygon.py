...
class PolygonMarker:
    ...
    def __init__(
        ...
        self.virtual_field_size = tuple(virtual_field_size)
    ...
    def _calculate_homography(self, src_points):
        ...
        # Целевой прямоугольник задаётся в .env, чтобы совпадать с расчётом миллиметров.
        width, height = self.virtual_field_size
    ...
    def _save_metadata(self):
        ...
        metadata = {
            ...
            "virtual_field_width": self.virtual_field_size[0],
            "virtual_field_height": self.virtual_field_size[1],
        }
        with metadata_path.open("w", encoding="utf-8") as metadata_file:
            json.dump(metadata, metadata_file, ensure_ascii=False, indent=2)
        print(f"✓ Размер кадра калибровки сохранён в {metadata_path}")

# Запускает отдельную разметку с камерой и размерами, которые указаны в общем .env.
def main():
