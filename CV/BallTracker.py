...
class BallTracker:
    ...
    def __init__(
        ...
        self.color_lower = np.array([0, 120, 120], dtype=np.uint8)
        self.color_upper = np.array([15, 255, 255], dtype=np.uint8)
        self.history_px = []
        self.max_history = 10
    ...
    def _detect_ball(self, frame):
        ...
        if not (3.0 < radius < 50.0 and area > 20.0 and circularity > 0.45):
            return None
